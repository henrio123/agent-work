...
function makeTempProjectsDir() {
  const dir = path.join(os.tmpdir(), `_test_ddeps_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);
...
function findItem(result, projectId, itemId) {
  const proj = result.projects.find(p => p.project_id === projectId);
...
test('epic has children listed', () => {
  ...
  makeProject(projectsDir, 'proj-c', [
    { id: 'EPIC-1', type: 'epic' },
    { id: 'CHILD-1', parent_id: 'EPIC-1' },
    { id: 'CHILD-2', parent_id: 'EPIC-1' },
    { id: 'OTHER' },
  ]);
  const result = buildDashboard({ projectsDir, workspaceRoot: projectsDir });
  const epic = findItem(result, 'proj-c', 'EPIC-1');
  ...
  const other = findItem(result, 'proj-c', 'OTHER');
...
test('depends_on_status shows unknown for dangling dep', () => {
  const projectsDir = makeTempProjectsDir();
  makeProject(projectsDir, 'proj-dangle', [
    { id: 'T-D', depends_on: ['GHOST'] },
  ]);
  const result = buildDashboard({ projectsDir, workspaceRoot: projectsDir });
  const td = findItem(result, 'proj-dangle', 'T-D');
...
test('empty depends_on_status for task with no deps', () => {
  ...
  const result = buildDashboard({ projectsDir, workspaceRoot: projectsDir });
  const t = findItem(result, 'proj-nd', 'T-ND');
...
test('blocked_by_deps lists non-done dependencies', () => {
  const projectsDir = makeTempProjectsDir();
  makeProject(projectsDir, 'proj-bd', [
    { id: 'D-1', status: 'done' },
    { id: 'D-2', status: 'todo' },
  ...
  const result = buildDashboard({ projectsDir, workspaceRoot: projectsDir });
  const t = findItem(result, 'proj-bd', 'T-BD');
