...
/**
 * Tests for project-next-drive.js — one-shot project-level driver.
...
function makeTempProjectsDir() {
  const dir = path.join(os.tmpdir(), `_test_proj_drv_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);
...
test('drive_skipped when empty projects', () => {
  const projectsDir = makeTempProjectsDir();
  const result = projectDriveOnce({ projectsDir, dryRun: true });
  if (result.action !== 'drive_skipped') throw new Error(`expected drive_skipped, got ${result.action}`);
});
...
test('dry_run skips when no run_folder exists', () => {
  const projectsDir = makeTempProjectsDir();
  makeProject(projectsDir, 'proj-dry', [
...
test('creates run folder when task has no run_folder', () => {
  const projectsDir = makeTempProjectsDir();
  makeProject(projectsDir, 'proj-create', [
  ...
  if (result.run_folder) {
    createdRunDirs.push(path.resolve(WORKSPACE_ROOT, result.run_folder));
  }
  ...
  // Verify run was created in real RUNS_DIR
  const absRun = path.resolve(WORKSPACE_ROOT, result.run_folder);
  ...
  const backlogItem = JSON.parse(fs.readFileSync(
    path.join(projectsDir, 'proj-create', 'backlog', 'T-NEW.json'), 'utf8'
  ));
...
test('drives existing run without creating new one', () => {
  ...
  // Create a real run folder for this test
  const runFolderName = `_test_proj_drv_existing_${Date.now()}`;
  const runFolder = `runs/${runFolderName}`;
  const absRun = path.join(RUNS_DIR, runFolderName);
  fs.mkdirSync(absRun, { recursive: true });
  createdRunDirs.push(absRun);

  // Set up a done status so autonomous runner finishes immediately
  ...
  makeProject(projectsDir, 'proj-exist', [
    { id: 'T-EXIST', status: 'in_progress', run_folder: runFolder },
  ]);

  // Note: classifyTask returns null for done runs, so this task won't be picked.
  // We need a non-done run instead.
  // Update: create a pm-ready run to be picked
  fs.writeFileSync(path.join(absRun, 'status.json'), JSON.stringify({
    ticket_id: 'T-EXIST',
    title: 'Existing task',
    project: 'proj-exist',
...
test('drive_created_run output validates against schema', () => {
  const projectsDir = makeTempProjectsDir();
  makeProject(projectsDir, 'proj-schema-new', [
...
test('backlog item gets last_summary after drive', () => {
  const projectsDir = makeTempProjectsDir();
  makeProject(projectsDir, 'proj-summary', [
  ...
  const backlogItem = JSON.parse(fs.readFileSync(
    path.join(projectsDir, 'proj-summary', 'backlog', 'T-SUM.json'), 'utf8'
  ));
